...
receita_estados = dados.drop_duplicates(subset="Local da compra")[['Local da compra', 'lat', 'lon']].merge(receita_estados,
                                                                                                           ...
                                                                                                           right_index=True).sort_values(by="Preço",
                                                                                                                                         ascending=False)
...
qtd_vendas = pd.DataFrame(dados.groupby("Local da compra")['Preço'].count())
qtd_vendas = dados.drop_duplicates(subset = 'Local da compra')[['Local da compra','lat', 'lon']].merge(qtd_vendas,
                                                                                                           left_on = 'Local da compra',
...
fig_mapa_vendas = px.scatter_geo(qtd_vendas, 
                     lat = 'lat', 
                     lon= 'lon', 
                     scope = 'south america', 
                     #fitbounds = 'locations', 
...
fig_vendas_categorias = px.bar(qtd_vendas_categoria, 
                                text_auto = True,
...
with aba1:
    ...
    with coluna1:
        st.metric(label="Receita", value=formata_numero(dados["Preço"].sum(), 'R$'))
        st.plotly_chart(fig_mapa_receita, use_container_width=True)
        st.plotly_chart(fig_receita_estados, use_container_width=True)
    with coluna2:
        st.metric(label="Quantidade de vendas", value=formata_numero(dados.shape[0]))
